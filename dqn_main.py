...
def select_action(state, number_of_bids):
    global steps_done
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            return policy_net(state.to('cuda'))
    else:
        return torch.tensor(np.random.uniform(size=(number_of_bids,)), device=device, dtype=torch.long)
